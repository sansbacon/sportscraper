...
class SportsAnalysis:
    ...
    def facet(self, df, cols, groupcol, hue=None):
        ...
        if hue:
            try:
                return sns.relplot(x=cols[0], y=cols[1], hue=hue, data=df, col=groupcol)
            except:
                pass
        return sns.relplot(x=cols[0], y=cols[1], col=groupcol, data=df)
    ...
    def scatter(self, df, cols, hue=None):
        ...
        if hue:
            try:
                return sns.relplot(x=cols[0], y=cols[1], data=df, hue=hue)
            except:
                pass
        return sns.relplot(x=cols[0], y=cols[1], data=df)
