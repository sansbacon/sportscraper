...
class Scraper(RequestScraper):
    ...
    def game(self, subresource="metadata", keys=None):
        ...
        if subresource not in self.game_subresources:
            ...
            if not keys:
                raise ValueError(
                    "cannot get subresource {} without keys".format(subresource)
                )
    ...
    def players(
        ...
        if league_id:
            ...
            url = (
                "https://fantasysports.yahooapis.com/fantasy/v2/teams;team_keys="
                + ",".join(team_keys)
                + "/players{}/{}"
            )
        elif player_keys:
            ...
            raise ValueError(
                "must specify one of: league_id, league_ids, team_key, team_keys, player_keys"
            )
    ...
    def transactions(self, league_id, subresource="metadata", filters=None):
        ...
        if subresource not in self.transactions_subresources:
            ...
            if set(filters.keys()) <= set(self.transactions_filters):
                ...
                return self.query(
                    url.format(self._league_key(league_id), self._filtstr(filters))
                )
            else:
                raise ValueError("games invalid filters: {}".format(filters))
        else:
