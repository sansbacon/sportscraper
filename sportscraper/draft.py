...
class Scraper(RequestScraper):
    ...
    def adp(self, start_date, end_date, season_year, participants="", entry_cost=""):
        """
    ...
    def bestball_ownership(self, file_name=None, window_cluster_id=None):
        ...
        if not content:
            url = (
                f"https://api.playdraft.com/v1/window_clusters/{window_cluster_id}/"
                f"ownerships?tournament=false"
            )
            content = self.get_json(url)
        return content

    def bestball_ownership_csv(
        self, file_name=None, window_cluster_id=None, sport=None
    ):
        """
    ...
    def complete_contests(self, file_name=None, window_cluster_id=None):
        ...
        if not content:
            url = (
                f"https://api.playdraft.com/v2/window_clusters/"
    ...
    def set_rankings(self, player_pool_id, bookings):
        ...
        headers = {
            ...
            "x-user-token": os.getenv("DRAFT_TOKEN"),
            "authority": "api.playdraft.com",
            "referer": f"https://draft.com/rankings/nfl/{player_pool_id}",
            "x-client-type": "web",
            "user-auth-token": os.getenv("DRAFT_TOKEN"),
        }

        url = "https://api.playdraft.com/v1/draft_rankings"
        data = json.dumps({"bookings": bookings, "player_pool_id": player_pool_id})
        response = self.post(url, data=data, headers=headers)
        return response.status_code
...
class Parser:
    ...
    def _combine_bookings_players(bookings, players):
        ...
        for p in players:
            ...
            if match:
                merged.append(
                    merge_two(
                        match, {k: v for k, v in p.items() if k in p_wanted}
                    )
                )
        return merged
    ...
    def bestball_ownership(self, content):
        ...
        for owned_player in content["ownerships"]:
            player_d = merge_two(
                owned_player, bookings_d.get(owned_player["booking_id"])
            )
            player_d["position"] = posd.get(player_d["position_id"])
            player_d["total_drafts"] = float(total_drafts)
            player_d["ownership_pct"] = round(
                player_d["total"] / player_d["total_drafts"], 3
            )
    ...
    def complete_contests(self, content):
        ...
        return [
            {
                "prize": float(dr["prize"]),
                "player_pool_id": dr["time_window_id"],
    ...
    def contest_results(self, content):
        ...
        contest_metadata = {k: v for k, v in contest.items() if k in contest_wanted}
        ...
        teams = [
            {k: v for k, v in team.items() if k in teams_wanted}
        ...
        users = [
            {k: v for k, v in user.items() if k in users_wanted}
    ...
    def draft_picks(self, draft):
        ...
        for t in rosters:
            for pick in [
                {k: v for k, v in pk.items() if k in pkwanted} for pk in t["picks"]
            ]:
                ...
                if match:
                    picks.append(merge_two(pick, match))
                else:
                    logging.info("no bookings match for %s" % pick)
        return picks
    ...
    def draft_users(self, draft):
        ...
        users = [
            {k: v for k, v in user.items() if k in uwanted} for user in draft["users"]
        ]
        league_users = [
            {
                "league_id": draft["id"],
...
class Agent:
    ...
    Usage:
        a = Agent()
        player_pool_id = 15035
        pool_date = sportscraper.utility.today()
        pp = a.player_pool(player_pool_id, pool_date)
        booking_ids = [p['booking_id'] for p in pp]
        new_bookings = random.sample(booking_ids[0:50], 25)
        result = a.s.set_rankings(player_pool_id, new_bookings)
    ...
    def bestball_leagues(self):
        ...
        try:
            ...
            bestball_cluster_id = [
                cl["id"]
                for cl in content["window_clusters"]
                if "Best Ball" in cl["header_text"]
            ][0]
